...
import { browserPreflightError } from "./browser.js";
...
async function serveCmd(): Promise<void> {
  ...
  // Same story for the browser: a missing/misplaced Chromium install only used to surface
  // when the first task ran (108ms into a swarm, deep in a stack trace). Checking now puts
  // it on /api/health at boot instead, next to credentialsError.
  const browserError = browserPreflightError();
  if (browserError) {
  ...
  controller.shoalServer.browserError = browserError;
